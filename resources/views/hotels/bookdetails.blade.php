        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="panel-group" id="accordion">
                    ...
                    @endforeach
                    <p><a href="{{route('invoice.view',$booking->id)}}" class="btn btn-info">View Invoice</a>
                        @if($booking->status == 'pending')
                            <a href="{{route('book.confirm',$booking->id)}}" class="btn btn-success">Confirm</a>
                        @endif
                        @if($booking->status == 'pending'||$booking->status == 'confirmed')
                            <a href="{{route('booking.cancel',$booking->id)}}" class="btn btn-danger">Cancel</a>
                        @endif
                        <a htef="#" class="btn btn-success">Change</a></p>
                </div>
            </div>
        </div>
    </div>
